!(function (factory) {
  ...
  var prop,
    ...
    mathXtra = {
      ...
      cbrt: function (x) {
        return 0 === x ? x : x < 0 ? -M.pow(-x, 1 / 3) : M.pow(x, 1 / 3);
      },
      ...
      trunc: function (x) {
        return 0 === x ? x : x < 0 ? M.ceil(x) : M.floor(x);
      },
      fround:
  ...
  for (prop in mathXtra)
    "function" != typeof M[prop] && (M[prop] = mathXtra[prop]);
  for (prop in numXtra)
    "function" != typeof N[prop] && (N[prop] = numXtra[prop]);
  try {
    (prop = {}), def(prop, 0, {});
    for (prop in numConsts)
      prop in N || def(N, prop, { value: numConsts[prop] });
  } catch (e) {
...
var $builtinmodule = function (name) {
  ...
  function get_ptr_simple(iter, coordinates) {
    var i, ret;
    for (ret = PyArray_DATA(iter.ao), i = 0; i < PyArray_NDIM(iter.ao); ++i)
  ...
  function PyArray_IterNew(obj) {
    ...
    return (
      (ao = obj),
      (it = Sk.abstr.iter(ao)),
      null == it ? null : (array_iter_base_init(it, ao), it)
    );
  }
  function PyArray_IterAllButAxis(obj, inaxis) {
    ...
    if (((arr = obj), (it = PyArray_IterNew(arr)), 0 == PyArray_NDIM(arr)))
      return it;
    if (inaxis < 0) {
      var i,
        minaxis = 0,
        minstride = 0;
      for (i = 0; 0 == minstride && i < PyArray_NDIM(arr); )
        (minstride = PyArray_STRIDE(arr, i)), (i += 1);
      for (i = 1; i < PyArray_NDIM(arr); i++)
        PyArray_STRIDE(arr, i) > 0 &&
          PyArray_STRIDE(arr, i) < minstride &&
          ((minaxis = 1), (minstride = PyArray_STRIDE(arr, i)));
      inaxis = minaxis;
    }
    return (
      (axis = inaxis),
      (it.contiguous = 0),
      0 != it.size && (it.size /= PyArray_DIM(arr, axis)),
      (it.dims_m1[axis] = 0),
  ...
  function _PyArray_ITER_NEXT2(it) {
    it.coordinates[1] < it.dims_m1[1]
      ? ((it.coordinates[1] += 1), (it.dataptr += it.strides[1]))
      : ((it.coordinates[1] = 0),
        (it.coordinates[0] += 1),
  ...
  function PyArray_ITER_NEXT(it) {
    if (((it.index += 1), 0 == it.nd_m1)) _PyArray_ITER_NEXT1(it);
      ...
      for (__npy_i = it.nd_m1; __npy_i >= 0; __npy_i--)
        it.coordinates[__npy_i] < it.dims_m1[__npy_i]
  ...
  function PyArray_UNPACK_SEQUENCE(seqObj) {
    if (Sk.builtin.checkSequence(seqObj)) {
      ...
      var i,
        ...
        ret = [];
      for (i = 0; i < length; i++)
        (item = seqObj.mp$subscript(i)), ret.push(item);
      return ret;
    }
    throw new Error(
      "Internal API-CAll error occured in PyArray_UNPACK_SEQUENCE"
    );
  }
  function PyArray_UNPACK_SHAPE(arr, shape) {
    ...
    if (Sk.builtin.checkInt(shape)) js_shape = [Sk.ffi.remapToJs(shape)];
      ...
      if (
        !Sk.builtin.checkSequence(shape) ||
        Sk.builtin.isinstance(shape, Sk.builtin.dict) != Sk.builtin.bool.false$
      )
    ...
    for (i = 0; i < js_shape.length; i++) {
      ...
      (js_shape[i] = Sk.ffi.remapToJs(js_shape[i])),
        js_shape[i] === -1 &&
          ((foundUnknownDimension += 1), (unknownDimensionIndex = i));
    }
    ...
    if (1 == foundUnknownDimension) {
      var knownDim, n_size;
      if (0 === unknownDimensionIndex)
        n_size = 1 === js_shape.length ? 1 : prod(js_shape.slice(1));
      else {
        var prod_shape = js_shape.slice();
        prod_shape.splice(unknownDimensionIndex, 1),
  ...
  function PyArray_NewShape(arr, shape, order) {
    ...
    throw new Error(
      "Internal API-Call Error occured in PyArray_NewShape.",
      arr
    );
  }
  ...
  function PyArray_Transpose(ap, permute) {
    ...
    if (null == permute)
      ...
      if (((n = permute.length), (axes = permute), n != PyArray_NDIM(ap)))
        throw new Sk.builtin.ValueError("axes don't match array");
      for (i = 0; i < n; i++) reverse_permutation[i] = -1;
      for (i = 0; i < n; i++) {
        if (
          ((axis = axes[i]),
          axis < 0 && (axis = PyArray_NDIM(ap) + axis),
          axis < 0 || axis >= PyArray_NDIM(ap))
        )
  ...
  function OBJECT_dot(ip1, is1, ip2, is2, op, n, ignore) {
    ...
    for (i = 0; i < n; i++, ip1_i += is1, ip2_i += is2) {
      if (null == ip1[ip1_i] || null == ip2[ip2_i])
        tmp1 = Sk.builtin.bool.false$;
      else if (
        ((tmp1 = Sk.abstr.numberBinOp(ip1[ip1_i], ip2[ip2_i], "Mult")), !tmp1)
      )
        return;
      if (0 == i) tmp = tmp1;
      else {
  ...
  function OBJECT_vdot(ip1, is1, ip2, is2, op, n, ignore) {
    ...
    for (i = 0; i < n; i++, ip1_i += is1, ip2_i += is2) {
      ...
      if (0 === i) tmp = tmp1;
        ...
        if (((tmp2 = Sk.abstr.numberBinOp(tmp, tmp1, "Add")), null == tmp2))
          return;
        tmp = tmp2;
      }
    }
  ...
  function PyArray_DTypeFromObject(obj, maxdims) {
    function seqUnpacker(obj, mDims) {
      ...
      if (Sk.builtin.asnum$(length) > 0) {
        ...
        return mDims > 0 && Sk.builtin.checkSequence(element)
          ? seqUnpacker(element, mDims - 1)
          : element;
      }
    }
    var dtype = null;
    ...
    if (Sk.builtin.checkNumber(obj)) element = obj;
      ...
      if (((element = seqUnpacker(obj, maxdims)), PyArray_Check(element)))
  ...
  function PyArray_ObjectType(op, minimum_type) {
    var dtype;
    if (((dtype = PyArray_DTypeFromObject(op, NPY_MAXDIMS)), null == dtype))
      return NPY_DEFAULT_TYPE;
    var num = Internal_DType_To_Num(dtype);
    return num >= minimum_type
      ? num
      : num < minimum_type
      ? num >= 0 && minimum_type <= 3
        ? minimum_type
        : NPY_NOTYPE
      : void 0;
  }
  function PyArray_Cast_Obj(obj, dtype) {
    return dtype instanceof Sk.builtin.none && Sk.builtin.checkNone(obj)
      ? obj
      : Sk.misceval.callsim(dtype, obj);
  }
  ...
  function PyArray_GetPriority(obj, default_) {
    ...
    return PyArray_CheckExact(obj)
      ? priority
      : PyArray_CheckAnyScalarExact(obj)
      ? NPY_SCALAR_PRIORITY
      : ((ret = Sk.builtin.getattr(
          obj,
          new Sk.builtin.str("__array_priority__"),
          Sk.builtin.none.none$
        )),
        Sk.builtin.checkNone(ret)
          ? default_
  ...
  function PyArray_NewFromDescr_int(
    ...
    if (nd > NPY_MAXDIMS)
      throw new Sk.builtin.ValueError(
        "number of dimensions must be within [0, " + NPY_MAXDIMS + "]"
      );
    if (((size = 1), (sd = 1), 0 == sd))
    ...
    for (i = 0; i < nd; i++) {
      ...
      if (null != dim) {
        ...
        if (((size = dim * size), size == Number.MAX_VALUE))
      ...
      null == data
        ? ((fa.flags = NPY_ARRAY_DEFAULT),
          flags &&
            ...
            nd > 1 && (fa.flags &= ~NPY_ARRAY_C_CONTIGUOUS),
  ...
  function _array_fill_strides(strides, dims, nd, itemsize, infalg, objflags) {
    ...
    for (i = nd - 1; i >= 0; i--)
      (strides[i] = itemsize),
        dims[i] ? (itemsize *= dims[i]) : (not_cf_contig = 0),
        1 == dims[i] && (strides[i] = NPY_MAX_INTP);
    return itemsize;
  }
  ...
  function new_array_for_sum(ap1, ap2, out, nd, dimensions, typenum) {
    ...
    if (
      (Sk.builtin.type(ap2) != Sk.builtin.type(ap1)
        ? ((prior = PyArray_GetPriority(ap2, 0)),
          (prior = PyArray_GetPriority(ap1, 0)),
          (subtype =
            prior2 > prior1 ? Sk.builtin.type(ap2) : Sk.builtin.type(ap1)))
        : ((prior1 = prior2 = 0), (subtype = Sk.builtin.type(ap1))),
      null != out)
    )
      throw new Error('new_array_for_sum does not support "out" parameter');
    return (ret = PyArray_New(
      subtype,
      nd,
      dimensions,
      typenum,
  ...
  function PyArray_FromAny(
    ...
    if (null == op)
      throw new Error(
        'Internal PyArray_FromAny API-Call error. "op" must not be null.'
      );
    (null == dtype || Sk.builtin.checkNone(dtype)) &&
      ((dtype = PyArray_DTypeFromObject(op, NPY_MAXDIMS)),
      null == dtype && (dtype = PyArray_DescrFromType(NPY_DEFAULT_TYPE)));
    var elements = [],
      state = {};
    for (
      state.level = 0,
  ...
  function convert_shape_to_string(n, vals, ending) {
    ...
    if (i === n)
      return Sk.abstr.numberBinOp(
        new Sk.builtin.str("()%s"),
        new Sk.builtin.str(ending),
        "Mod"
      );
    for (
      ...
      (tmp =
        vals[i] < 0
          ? new Sk.builtin.str(",newaxis")
          : Sk.abstr.numberBinOp(new Sk.builtin.str(",%i"), vals[i], "Mod")),
  ...
  function dot_alignment_error(a, i, b, j) {
    ...
    if (
      ...
      format && shape1 && shape2 && i_obj && j_obj && shape1_i && shape2_j)
    )
      throw (
        ((fmt_args = new Sk.builtin.tuple([
          shape1,
          shape2,
          shape1_i,
          i_obj,
        ...
        null != errmsg
          ? new Sk.builtin.ValueError(errmsg)
          : new Sk.builtin.ValueError("shapes are not aligned"))
      );
  }
  function MatrixProdcut2(op1, op2, out) {
    ...
    if (
      ...
      0 == PyArray_NDIM(ap1) || 0 == PyArray_NDIM(ap2))
    )
      return (
        (ret = 0 == PyArray_NDIM(ap1) ? ap1 : ap2),
        (ret = ret.nb$multiply.call(ap1, ap2))
      );
    if (
      ((l = PyArray_DIMS(ap1)[PyArray_NDIM(ap1) - 1]),
      (matchDim = PyArray_NDIM(ap2) > 1 ? PyArray_NDIM(ap2) - 2 : 0),
      PyArray_DIMS(ap2)[matchDim] != l &&
    ...
    for (j = 0, i = 0; i < PyArray_NDIM(ap1) - 1; i++)
    ...
    if (
      ...
      return null;
    if (((it2 = PyArray_IterAllButAxis(ap2, matchDim)), null == it2))
      return null;
    for (
      var it1DeRefDataPtr, it2DeRefDataPtr, op_i = 0;
      it1.index < it1.size;
    ...
    if (0 === nd)
      ...
      for (strPtr.str += "[", nPtr.n += 1, i = 0; i < dimensions[0]; i++) {
        ...
        dump_data(
          ...
          i < dimensions[0] - 1 &&
            ((strPtr.str += ","), (strPtr.str += " "), (nPtr.n += 2));
      }
      (strPtr.str += "]"), (nPtr.n += 1);
    }
    return ret;
  ...
  function array_repr_builtin(self, repr) {
    ...
    return (
      ...
      repr
        ? (PyTypeNum_ISEXTENDED(self)
            ? ((format = new Sk.builtin.str("array(%s, '%s')")),
  ...
  function remapToJs_shallow(obj, shallow) {
    ...
    if (obj instanceof Sk.builtin.list) {
      ...
      for (var ret = [], i = 0; i < obj.v.length; ++i)
        ret.push(Sk.ffi.remapToJs(obj.v[i]));
      return ret;
    }
    return obj instanceof Sk.builtin.float_
      ? Sk.builtin.asnum$nofloat(obj)
      : Sk.ffi.remapToJs(obj);
  }
  function unpack(py_obj, buffer, state) {
    if (
      (PyArray_Check(py_obj) &&
        (py_obj = Sk.misceval.callsim(py_obj.tolist, py_obj)),
      py_obj instanceof Sk.builtin.list || py_obj instanceof Sk.builtin.tuple)
    ) {
      var py_items = remapToJs_shallow(py_obj);
      (state.level += 1),
        state.level > state.shape.length && state.shape.push(py_items.length);
      var i,
        len = py_items.length;
      for (i = 0; i < len; i++) unpack(py_items[i], buffer, state);
      state.level -= 1;
  ...
  function computeStrides(shape) {
    ...
    strides.reverse();
    for (var temp, prod = 1, i = 0, len = strides.length; i < len; i++)
      (temp = strides[i]), (strides[i] = prod), (prod *= temp);
    return strides.reverse();
  }
  function computeOffset(strides, index) {
    for (var offset = 0, k = 0, len = strides.length; k < len; k++)
  ...
  function tolistrecursive(buffer, shape, strides, startdim, dtype) {
    ...
    for (
      n = shape[startdim], stride = strides[startdim], arr = [], i = 0;
      i < n;
      i++
    )
  ...
  function callTrigonometricFunc(x, op) {
    var res, num;
    if (
      ((x instanceof Sk.builtin.list || x instanceof Sk.builtin.tuple) &&
      ...
      var _buffer = PyArray_DATA(x).map(function (value) {
          return (
            ...
            new Sk.builtin.float_(res)
          );
        }),
        shape = new Sk.builtin.tuple(
          PyArray_DIMS(x).map(function (d) {
            return new Sk.builtin.int_(d);
          })
        );
      return (
        (buffer = new Sk.builtin.list(_buffer)),
        Sk.misceval.callsim(mod[CLASS_NDARRAY], shape, PyArray_DESCR(x), buffer)
      );
    }
  ...
  function compareLogical(binOp, x1, x2, out) {
    ...
    if (
      !(
        PyArray_Check(x1) ||
        Sk.builtin.checkSequence(x1) ||
        PyArray_Check(x2) ||
        Sk.builtin.checkSequence(x2)
      )
    )
    ...
    if (void 0 != out && !Sk.builtin.checkNone(out))
      throw new ValueError('"out" parameter not supported');
    for (i = 0; i < data1.length; i++)
      buf.push(
        Sk.builtin.bool(Sk.misceval.richCompareBool(data1[i], data2[i], binOp))
      );
    return (
  ...
  numpy.prototype.arange = function (start, stop, step) {
    ...
    for (var res = [], i = start; i < stop; i += step) res.push(i);
    return res;
  };
  var NPY_NOTYPE = null,
    NPY_DEFAULT_TYPE = 2,
  ...
  var ndarray_f = function ($gbl, $loc) {
    function makeNumericBinaryOpLhs(operation) {
      return function (self, other) {
        ...
        if (PyArray_Check(other))
          ...
          for (
            lhs = PyArray_DATA(self), _buffer = [], i = 0, len = lhs.length;
            i < len;
            i++
          )
        ...
        return (
          (shape = new Sk.builtin.tuple(
            PyArray_DIMS(self).map(function (x) {
              return new Sk.builtin.int_(x);
            })
          )),
          (buffer = new Sk.builtin.list(_buffer)),
    ...
    function makeNumericBinaryOpInplace(operation) {
      return function (self, other) {
        ...
        if (PyArray_Check(other))
          ...
          for (lhs = PyArray_DATA(self), i = 0, len = lhs.length; i < len; i++)
    ...
    ($loc.__init__ = new Sk.builtin.func(function (
      ...
      (ndarrayJs.shape = Sk.ffi.remapToJs(shape)),
        ...
        (ndarrayJs.flags = 0),
        buffer &&
          buffer instanceof Sk.builtin.list &&
          (ndarrayJs.buffer = buffer.v),
        (self.v = ndarrayJs),
        (self.tp$name = CLASS_NDARRAY);
    })),
      ($loc._internalGenericGetAttr =
        Sk.builtin.object.prototype.GenericSetAttr),
      ($loc.__getattr__ = new Sk.builtin.func(function (self, name) {
        if (
          null != name &&
          (Sk.builtin.checkString(name) || "string" == typeof name)
        ) {
          var _name = name;
          switch (
            (Sk.builtin.checkString(name) && (_name = Sk.ffi.remapToJs(name)),
            _name)
          ) {
            case "ndim":
            ...
            case "strides":
              return new Sk.builtin.tuple(
                PyArray_STRIDES(self).map(function (x) {
                  return new Sk.builtin.int_(x);
                })
              );
            case "size":
            ...
            case "T":
              return PyArray_NDIM(self) < 2
                ? self
                : Sk.misceval.callsim(self.transpose, self);
          }
        }
        var r, f, ret;
        if (
          (void 0 !== self.tp$getattr &&
          ...
          switch (
            ...
            case "shape":
              ...
              return (
                ...
                void (self.v.shape = js_shape)
              );
          }
        }
        throw new Sk.builtin.AttributeError(
          "'ndarray' object attribute '" + name + "' is readonly"
        );
      })),
      ...
      ($loc.copy = new Sk.builtin.func(function (self, order) {
        ...
        var buffer =
            (Sk.ffi.remapToJs(self),
            PyArray_DATA(self).map(function (x) {
              return x;
            })),
          shape = new Sk.builtin.tuple(
            PyArray_DIMS(self).map(function (x) {
      ...
      ($loc.fill = new Sk.builtin.func(function (self, value) {
        ...
        var i,
          ndarrayJs = Sk.ffi.remapToJs(self);
      ...
      ($loc.__getslice__ = new Sk.builtin.func(function (self, start, stop) {
        ...
        if (
          ...
          Sk.builtin.checkInt(stop) ||
          Sk.builtin.checkNone(stop) ||
          void 0 === stop
        )
          throw new Sk.builtin.ValueError('Index "' + _index + '" must be int');
        if (
          ((_start = Sk.ffi.remapToJs(start)),
          (_stop =
            void 0 === stop || Sk.builtin.checkNone(stop)
              ? ndarrayJs.buffer.length
              : Sk.ffi.remapToJs(start)),
          _start < 0 || _stop < 0)
        )
          throw new Sk.builtin.IndexError(
            "Use of negative indices is not supported."
          );
        for (buffer_internal = [], _index = 0, i = _start; i < _stop; i += 1)
      ...
      ($loc.__setslice__ = new Sk.builtin.func(function (
        self,
        start,
        stop,
        value
        ...
        if (
          ...
          throw new Sk.builtin.ValueError('Index "' + index + '" must be int');
        if (
          ((_start = Sk.ffi.remapToJs(start)),
          (_stop =
            void 0 === stop || Sk.builtin.checkNone(stop)
      ...
      ($loc.__getitem__ = new Sk.builtin.func(function (self, index) {
        ...
        if (index instanceof Sk.builtin.tuple) {
          ...
          if (keyJs.length != PyArray_NDIM(self))
            throw new Sk.builtin.ValueError(
              "Tuple must contain values for all dimensions"
            );
      ...
      ($loc.__str__ = new Sk.builtin.func(function (self) {
        return null == PyArray_StrFunction
          ? Sk.misceval.callsim(self.__repr__, self)
          : PyArray_StrFunction.call(null, self);
      })),
      ...
      ($loc.__floordiv__ = new Sk.builtin.func(
        makeNumericBinaryOpLhs("FloorDiv")
      )),
      ($loc.__rfloordiv__ = new Sk.builtin.func(
        makeNumericBinaryOpRhs("FloorDiv")
      )),
      ($loc.__ifloordiv__ = new Sk.builtin.func(
        makeNumericBinaryOpInplace("FloorDiv")
      )),
      ...
      ($loc.__ixor__ = new Sk.builtin.func(
        makeNumericBinaryOpInplace("BitXor")
      )),
      ($loc.__lshift__ = new Sk.builtin.func(makeNumericBinaryOpLhs("LShift"))),
      ($loc.__rlshift__ = new Sk.builtin.func(
        makeNumericBinaryOpRhs("LShift")
      )),
      ($loc.__ilshift__ = new Sk.builtin.func(
        makeNumericBinaryOpInplace("LShift")
      )),
      ($loc.__rshift__ = new Sk.builtin.func(makeNumericBinaryOpLhs("RShift"))),
      ($loc.__rrshift__ = new Sk.builtin.func(
        makeNumericBinaryOpRhs("RShift")
      )),
      ($loc.__irshift__ = new Sk.builtin.func(
        makeNumericBinaryOpInplace("RShift")
      )),
      ...
      ($loc.transpose = new Sk.builtin.func(function (self, args) {
        var ret,
          shape = Sk.builtin.none.none$,
          n = arguments.length - 1,
          permute = [];
        return (
          (args = Array.prototype.slice.call(arguments, 1)),
          n > 1 ? (shape = args) : 1 === n && (shape = args[0]),
          Sk.builtin.checkNone(shape)
      ...
      ($loc.mean = new Sk.builtin.func(function (
        self,
        axis,
        dtype,
        out,
      ...
      ($loc.sum = new Sk.builtin.func(function (
        self,
        axis,
        dtype,
        out,
      ...
      ($loc.prod = new Sk.builtin.func(function (
        self,
        axis,
        dtype,
        out,
  ...
  (mod[CLASS_NDARRAY] = Sk.misceval.buildClass(
    mod,
    ndarray_f,
    CLASS_NDARRAY,
    []
  ...
  var sin_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.sin : Math.sin)
    );
  };
  ...
  var sinh_f = function (x, out) {
    if ((Sk.builtin.pyCheckArgs("sinh", arguments, 1, 2), !np.math))
      throw new Sk.builtin.OperationError("sinh requires math polyfill");
  ...
  var arcsin_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.asin : Math.asin)
    );
  };
  ...
  var cos_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.cos : Math.cos)
    );
  };
  ...
  var cosh_f = function (x, out) {
    if ((Sk.builtin.pyCheckArgs("cosh", arguments, 1, 2), !np.math))
      throw new Sk.builtin.OperationError("cosh requires math polyfill");
  ...
  var arccos_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.acos : Math.acos)
    );
  };
  ...
  var arctan_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.atan : Math.atan)
    );
  };
  ...
  var tan_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.tan : Math.tan)
    );
  };
  ...
  var tanh_f = function (x, out) {
    if ((Sk.builtin.pyCheckArgs("tanh", arguments, 1, 2), !np.math))
      throw new Sk.builtin.OperationError("tanh requires math polyfill");
  ...
  var exp_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.exp : Math.exp)
    );
  };
  ...
  var sqrt_f = function (x, out) {
    return (
      ...
      callTrigonometricFunc(x, np.math ? np.math.sqrt : Math.sqrt)
    );
  };
  ...
  var linspace_f = function (start, stop, num, endpoint, retstep) {
    Sk.builtin.pyCheckArgs("linspace", arguments, 3, 5),
      ...
      void 0 === num && (num = 50);
    var endpoint_bool,
      num_num = Sk.builtin.asnum$(num);
    void 0 === endpoint
      ? (endpoint_bool = !0)
      : endpoint.constructor === Sk.builtin.bool &&
        (endpoint_bool = endpoint.v);
    var retstep_bool;
    void 0 === retstep
      ? (retstep_bool = !1)
      : retstep.constructor === Sk.builtin.bool && (retstep_bool = retstep.v);
    var samples, step;
    if (
      ((start_num = 1 * Sk.builtin.asnum$(start)),
      (stop_num = 1 * Sk.builtin.asnum$(stop)),
      num_num <= 0)
    )
      samples = [];
    else {
    ...
    return retstep_bool === !0
      ? new Sk.builtin.tuple([ndarray, step])
      : ndarray;
  };
  (linspace_f.co_varnames = ["start", "stop", "num", "endpoint", "retstep"]),
    (linspace_f.$defaults = [0, 0, 50, !0, !1]),
  ...
  var arange_f = function (start, stop, step, dtype) {
    ...
    void 0 === stop && void 0 === step
      ...
      (dtype && dtype != Sk.builtin.none.none$) ||
        (dtype = Sk.builtin.checkInt(start)
          ? Sk.builtin.int_
          : Sk.builtin.float_);
  ...
  var array_f = function (object, dtype, copy, order, subok, ndmin) {
    if ((Sk.builtin.pyCheckArgs("array", arguments, 1, 6), void 0 === object))
      throw new Sk.builtin.TypeError(
        "'" + Sk.abstr.typeName(object) + "' object is undefined"
      );
    if (null != ndmin && Sk.builtin.checkInt(ndmin) === !1)
  ...
  var zeros_f = function (shape, dtype, order) {
    if (
      ...
      !Sk.builtin.checkSequence(shape) && !Sk.builtin.checkInt(shape))
    )
      throw new Sk.builtin.TypeError(
        'argument "shape" must int or sequence of ints'
      );
  ...
  var full_f = function (shape, fill_value, dtype, order) {
    ...
    if (dtype instanceof Sk.builtin.list)
      throw new Sk.builtin.TypeError(
        "'" +
          Sk.abstr.typeName(dtype) +
          "' is currently not supported for dtype."
      );
    var _tup,
      _shape = Sk.ffi.remapToJs(shape);
    Sk.builtin.checkInt(shape) &&
      ((_tup = []), _tup.push(_shape), (_shape = _tup));
    var i,
      _size = prod(_shape),
      _buffer = [],
      _fill_value = fill_value;
    for (i = 0; i < _size; i++) _buffer[i] = _fill_value;
    if (
      (!dtype &&
        Sk.builtin.checkNumber(fill_value) &&
        (dtype = Sk.builtin.float_),
      Sk.builtin.checkClass(dtype))
    )
      for (i = 0; i < _buffer.length; i++)
        _buffer[i] !== Sk.builtin.none.none$ &&
          (_buffer[i] = Sk.misceval.callsim(dtype, _buffer[i]));
    return Sk.misceval.callsim(
      mod[CLASS_NDARRAY],
      new Sk.builtin.list(_shape),
      dtype,
      new Sk.builtin.list(_buffer)
    );
  };
  ...
  var abs_f = function (x) {
    ...
    return (ret =
      1 == PyArray_Check(x)
        ? Sk.misceval.callsim(x.__abs__, x)
        : Sk.builtin.abs(x));
  };
  ...
  var mean_f = function (x, axis, dtype, out, keepdims) {
    ...
    var mean,
      _buffer,
      length,
      sum = new Sk.builtin.float_(0),
      i = 0;
    if (null != axis && !Sk.builtin.checkNone(axis))
      throw new Sk.builtin.NotImplementedError(
        "the 'axis' parameter is currently not supported"
      );
    if (null != out && !Sk.builtin.checkNone(out))
      throw new Sk.builtin.NotImplementedError(
        "the 'out' parameter is currently not supported"
      );
    if (null != keepdims && keepdims != Sk.builtin.bool.false$)
      throw new Sk.builtin.NotImplementedError(
        "the 'keepdims' parameter is currently not supported"
      );
  ...
  var dot_f = function (a, b, o) {
    ...
    var o, ret;
    if ((Sk.builtin.checkNone(o) && (o = null), null != o && !PyArray_Check(o)))
  ...
  var vdot_f = function (a, b) {
    ...
    if (((op1 = PyArray_NewShape(ap1, newdims, "NPY_CORDER")), null != op1)) {
      ...
      if (((op2 = PyArray_NewShape(ap2, newdims, "NPY_CORDER")), null != op2)) {
        if (((ap2 = op2), PyArray_DIM(ap2, 0) != PyArray_DIM(ap1, 0)))
  ...
  var any_f = function (a, axis, out, keepdims) {
    ...
    var i,
      b,
      arr = PyArray_FromAny(a),
      data = PyArray_DATA(arr);
    if (void 0 != axis && !Sk.builtin.checkNone(axis))
      throw new ValueError('"axis" parameter not supported');
    if (void 0 != out && !Sk.builtin.checkNone(out))
      throw new ValueError('"out" parameter not supported');
    for (i = 0; i < data.length; i++)
      if (((b = Sk.builtin.bool(data[i])), b == Sk.builtin.bool.true$))
  ...
  var all_f = function (a, axis, out, keepdims) {
    ...
    if (void 0 != out && !Sk.builtin.checkNone(out))
      throw new ValueError('"out" parameter not supported');
    for (i = 0; i < data.length; i++)
      if (((b = Sk.builtin.bool(data[i])), b == Sk.builtin.bool.false$))
        return Sk.builtin.bool.false$;
    return Sk.builtin.bool.true$;
  };
  ...
  var less_f = function (x1, x2, out) {
    return (
      Sk.builtin.pyCheckArgs("less", arguments, 2, 3, !1),
      compareLogical("Lt", x1, x2, out)
    );
  };
  ...
  var less_equal_f = function (x1, x2, out) {
    return (
      Sk.builtin.pyCheckArgs("less_equal", arguments, 2, 3, !1),
      compareLogical("LtE", x1, x2, out)
    );
  };
  (less_equal_f.co_varnames = ["a", "axis", "out", "keepdims"]),
    (less_equal_f.$defaults = [
      Sk.builtin.none.none$,
      Sk.builtin.none.none$,
      Sk.builtin.bool.false$,
    ]),
  ...
  var greater_f = function (x1, x2, out) {
    return (
      Sk.builtin.pyCheckArgs("greater", arguments, 2, 3, !1),
      compareLogical("Gt", x1, x2, out)
    );
  };
  ...
  var greater_equal_f = function (x1, x2, out) {
    return (
      Sk.builtin.pyCheckArgs("greater_equal", arguments, 2, 3, !1),
      compareLogical("GtE", x1, x2, out)
    );
  };
  (greater_equal_f.co_varnames = ["a", "axis", "out", "keepdims"]),
    (greater_equal_f.$defaults = [
      Sk.builtin.none.none$,
      Sk.builtin.none.none$,
      Sk.builtin.bool.false$,
    ]),
  ...
  var equal_f = function (x1, x2, out) {
    return (
      Sk.builtin.pyCheckArgs("equal", arguments, 2, 3, !1),
      compareLogical("Eq", x1, x2, out)
    );
  };
  ...
  var not_equal_f = function (x1, x2, out) {
    return (
      Sk.builtin.pyCheckArgs("not_equal", arguments, 2, 3, !1),
      compareLogical("NotEq", x1, x2, out)
    );
  };
  return (
    (not_equal_f.co_varnames = ["a", "axis", "out", "keepdims"]),
    (not_equal_f.$defaults = [
      Sk.builtin.none.none$,
      Sk.builtin.none.none$,
      Sk.builtin.bool.false$,
    ]),
